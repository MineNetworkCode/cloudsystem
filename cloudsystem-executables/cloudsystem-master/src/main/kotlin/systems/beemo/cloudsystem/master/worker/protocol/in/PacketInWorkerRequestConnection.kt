package systems.beemo.cloudsystem.master.worker.protocol.`in`
...
class PacketInWorkerRequestConnection : Packet {
    ...
    override fun read(document: Document) {
        ...
        verified = secretKey == CloudSystemMaster.RUNTIME_VARS.secretKey
    ...
    override fun handle(channelHandlerContext: ChannelHandlerContext) {
        ...
        networkUtils.sendPacket(
            PacketOutWorkerConnectionEstablished(
                ...
                webKey = CloudSystemMaster.RUNTIME_VARS.webKey,
                successful = true
            ), channelHandlerContext.channel()
        )
        logger.info(
        ...
        // TODO: Request process on startup
    }
    ...
    private fun isWorkerAuthenticated(channel: Channel, workerName: String): Boolean {
        ...
        val validWorkers = CloudSystemMaster.RUNTIME_VARS.masterConfig.validWorkers
